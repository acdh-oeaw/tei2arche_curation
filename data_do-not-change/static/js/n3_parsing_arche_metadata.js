    ...
    if ($(".loading_animation_head").length !== 0) {
        ...
        if ($(".loading_animation_head").length !== 0) {
            ...
            if (localStorage.getItem(project_title.replace(".xsl","") + "_metadata") == null) {
                ...
                var resourceId = $('#root_col_url').text().replace('https://arche.acdh.oeaw.ac.at/api/','');
                var readMode = 'neighbors';
                downloader(host, resourceId, format, readMode, (rs) => {
                    var subject = $('#root_col_url').text();  
                    var predicate = null;
                    var object = null;
                    ...
                    for (var i = 0; i < topCollection.length; i++) {
                        if (topCollection[i].object.includes('@de') || !topCollection[i].object.includes('@')){
                            if (topCollection[i].predicate == arche + "hasTitle") {                            
                                $("#metadata_title").append(`<h2>${topCollection[i].object.replace(regex,'')}</h2>`);
                            }
                            else if (topCollection[i].predicate == arche + "hasDescription") {
                                $("#metadata_description").append(`<p>${topCollection[i].object.replace(regex,'')}</p>`);
                            }
                            else if (topCollection[i].predicate == arche + "hasSubject") {
                                $("#metadata_keywords").append(`<li>${topCollection[i].object.replace(regex,'')}</li>`);              
                            }
                            else if (topCollection[i].predicate == arche + "hasAccessRestrictionSummary") {
                                accessRestrictions += `<td>${topCollection[i].object.replace(regex,'')}</td>`;          
                            }
                        }    
                        if (topCollection[i].predicate == arche + "hasCreatedStartDate") {
                            ...
                            owners += `<td><a target="_blank" href="${topCollection[i].object.replace('/api/',browser)}">${owner[0].object.replace(regex,'')}</a></td>`; 

                        }
                        else if (topCollection[i].predicate == arche + "hasContact") {
                            var contact = query(topCollection[i].object, arche + "hasTitle", null, rs);
                            contacts += `<td><a target="_blank" href="${topCollection[i].object.replace('/api/',browser)}">${contact[0].object.replace(regex,'')}</a></td>`; 
                        }
                            ...
                            for (e = 0; e < contributor.length; e++) {
                                if (contributor[e].predicate == arche + "hasTitle") {
                                    contributors += `<td><a target="_blank" href="${topCollection[i].object.replace('/api/',browser)}">${contributor[e].object.replace(regex,'')}</a></td>`; 
                                }
                            }             
                        }
                        else if (topCollection[i].predicate == arche + "hasCreator") {
                            var creator = query(topCollection[i].object, arche + "hasTitle", null, rs);
                            creators += `<td><a target="_blank" href="${topCollection[i].object.replace('/api/',browser)}">${creator[0].object.replace(regex,'')}</a></td>`;          
                        }
                        else if (topCollection[i].predicate == arche + "hasCurator") {
                            var curator = query(topCollection[i].object, arche + "hasTitle", null, rs);
                            curators += `<td><a target="_blank" href="${topCollection[i].object.replace('/api/',browser)}">${curator[0].object.replace(regex,'')}</a></td>`;             
                        }
                        else if (topCollection[i].predicate == arche + "hasLicensor") {
                            var licensor = query(topCollection[i].object, arche + "hasTitle", null, rs);
                            licensors += `<td><a target="_blank" href="${topCollection[i].object.replace('/api/',browser)}">${licensor[0].object.replace(regex,'')}</a></td>`;           
                        }
                        else if (topCollection[i].predicate == arche + "hasLicense") {
                            var license = query(topCollection[i].object, arche + "hasTitle", null, rs);
                            licenses += `<td><a target="_blank" href="${topCollection[i].object.replace('/api/',browser)}">${license[0].object.replace(regex,'')}</a></td>`;            
                        }
                            ...
                            relatedDisciplines += `<td><a target="_blank" href="${topCollection[i].object.replace('/api/',browser)}">${relatedDiscipline[0].object.replace(regex,'')}</a></td>`;            
                        }
                            ...
                            relatedProjects += `<td><a target="_blank" href="${topCollection[i].object.replace('/api/',browser)}">${relatedProject[0].object.replace(regex,'')}</a></td>`;           
                        }                        
                    }
